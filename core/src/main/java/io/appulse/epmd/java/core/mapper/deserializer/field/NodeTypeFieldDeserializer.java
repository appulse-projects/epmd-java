...
public final class NodeTypeFieldDeserializer extends AbstractFieldDeserializer<NodeType> {

  @Getter(lazy = true, value = PRIVATE)
  NumberFieldDeserializer numberFieldDeserializer = createNumberFieldDeserializer();

  @Override
  public NodeType read (byte[] bytes, FieldDescriptor descriptor) {
    byte code = (byte) getNumberFieldDeserializer().read(bytes, Byte.class);
    return NodeType.of(code);
  }

  @Override
  boolean isApplicable (FieldDescriptor descriptor) {
    return NodeType.class.isAssignableFrom(descriptor.getType());
  }
